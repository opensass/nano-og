...
pub fn ChatPanel(conversation_id: Signal<ObjectId>, user_token: Signal<String>) -> Element {
    ...
    let _ = use_resource(move || async move {
        ...
        if let Ok(cached_data) = LocalStorage::get::<CachedOGsData>(CACHE_KEY) {
            if now - cached_data.timestamp < CACHE_TIMEOUT {
                ...
                if let Some(first_og) = cached_data.data.first() {
                    selected_og.set(Some(first_og.clone()));
                }
                return;
            }
        }
        ...
        .await
        {
            Ok(response) => {
                ...
                if let Some(first_og) = response.data.first() {
                    selected_og.set(Some(first_og.clone()));
                }
            }
            Err(_) => {
    ...
    let mut handle_og_change = move |og_id: String| {
